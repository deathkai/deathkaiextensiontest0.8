...
var _Sources = (() => {
  ...
  // src/GenzUpdates/components/LanguageUtils.ts
  ...
  // src/GenzUpdates/GenzUpdatesParser.ts
  var import_html_entities = __toESM(require_lib2());

  // src/GenzUpdates/components/Helper.ts
...
Image url: ${image}`
  ...
  // src/GenzUpdates/components/SourceRequestManager.ts
  ...
  // src/GenzUpdates/components/Types.ts
  ...
  var GenzUpdatesInfo = {
    version: "2.0.15",
    name: "GenzUpdates",
    description: "Extension that pulls manga from GenzUpdates or it's derivatives.",
    author: "Deathkai",
    authorWebsite: "http://github.com/deathkai",
    icon: "icon.png",
  ...
  var GenzUpdates = class _GenzUpdates extends Configuration {
    constructor() {
      super();
      this.homescreen_sections = {
        popular_today: {
          ...
          getViewMoreItemsFunc: (page) => `${_GenzUpdates.directoryPath}/?page=${page}&order=popular`,
          sortIndex: 10
        },
        latest_update: {
          ...DefaultHomeSectionData,
          ...
          // TODO: Remove nicely
          // getViewMoreItemsFunc: (page: string) => `${GenzUpdates.directoryPath}/?page=${page}&order=update`,
      ...
      this.requestManager = getSourceRequestManager(_GenzUpdates.baseUrl);
    ...
    getMangaShareUrl(mangaTitle) {
      return `${_GenzUpdates.baseUrl}/${_GenzUpdates.directoryPath}/${getSlugFromTitle(mangaTitle)}/`;
    }
    async getMangaDetails(mangaTitle) {
      const mangaId = getSlugFromTitle(mangaTitle);
      const request = App.createRequest({
        url: `${_GenzUpdates.baseUrl}/${_GenzUpdates.directoryPath}/${mangaId}/`,
        method: "GET"
      });
    ...
    async getSearchTags() {
      const request = App.createRequest({
        url: `${_GenzUpdates.baseUrl}/${_GenzUpdates.filterPath}/`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.checkResponseError(response);
      ...
      for (const manga of searchResultData) {
        results.push(
          App.createPartialSourceManga({
            ...
            title: manga.title,
            image: `${_GenzUpdates.baseUrl}/assets/images/${manga.image_url}`
          })
        );
      }
    ...
    async constructSearchRequest(page, query) {
      let searchUrl = new URLBuilder(_GenzUpdates.baseUrl);
      const headers = {
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        origin: _GenzUpdates.baseUrl
      };
      const formData = {};
      if (query?.title) {
        searchUrl = searchUrl.addPathComponent(_GenzUpdates.searchEndpoint);
        formData["search_value"] = query?.title.replace(/[’–][a-z]*/g, "") ?? "";
      } else {
        searchUrl = searchUrl.addPathComponent(_GenzUpdates.filterEndpoint);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (homepageSectionId) {
        case "latest_update": {
          ...
          const request = App.createRequest({
            url: `${_GenzUpdates.baseUrl}/${_GenzUpdates.filterEndpoint}`,
            headers,
            data: Object.entries(formData).map(
        ...
        default: {
          ...
          const request = App.createRequest({
            url: `${_GenzUpdates.baseUrl}/${param}`,
            method: "GET"
          });
          const response = await this.requestManager.schedule(request, 1);
          const $2 = load(response.data);
    ...
    async getCloudflareBypassRequestAsync() {
      ...
      return App.createRequest({
        url: `${_GenzUpdates.bypassPage || _GenzUpdates.baseUrl}/`,
        method: "GET",
        headers: {
          referer: `${_GenzUpdates.baseUrl}/`,
          origin: `${_GenzUpdates.baseUrl}/`,
...
Please go to the homepage of <${_GenzUpdates.baseUrl}> and press the cloud icon.`
          );
        case 404:
          throw new Error(
  ...
  return __toCommonJS(GenzUpdates_exports);
})();
